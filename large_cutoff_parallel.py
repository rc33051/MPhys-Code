...
def zeta_sum_parallel(q_2_star=1.5, cutoff=9, alpha=1, d=np.array([0, 0, 0]), m_tilde_sq=(4/np.pi)**2, beta_scalar=0, gamma=1):
        ...
        for j in range(len(A[i])):
            for k in range(len(A[i, j])):
                ...
                if np.sum(np.min([lower_bounds**2, upper_bounds**2], axis=0)) <= cutoff:
                    tasks.append((lower_bounds, upper_bounds, q_2_star, cutoff, alpha, beta_scalar, beta_norm, gamma, m_tilde_sq))

    results = []
    with ProcessPoolExecutor() as executor:
        results = list(tqdm(executor.map(unpack_evaluate_terms_summand, tasks), total=len(tasks)))
...
def zeta(q_2_star=1.5, cutoff=9, alpha=-1, d = np.array([0,0,0])):
    ...
    sum_result  = zeta_sum_parallel(q_2_star,cutoff, recommended_alpha, d, m_tilde_sq, beta, gamma)
...
# Ensure the rest of your functions (evaluate_terms_summand, zeta_pv, zeta) are defined here